function WebOffice2015() {
    ...
    */
    ...
    this.WebSave = function () {
        ...
        httpclient.Clear();
        try{
            WebOffice.VBAUnProtectDocument(WebOffice.getDocType(WebOffice.FileType), "123")
        }catch (e) {
        }
        var date = new Date().getTime();
        this.FileName = "文档保存" + date + this.FileType;
        ...
        if (httpclient.Open(this.HttpMethod.Post, this.WebUrl, false)) //true 异步方式 false同步
        {
            ...
            if (httpclient.Send()) {
                ...
                if (this.hiddenSaveLocal(httpclient, this, false, false)) {
                    ...
                    if (this.OpenLocalFile(this.DownloadedFileTempPathName) == 0) { //打开本地磁盘文件
                        this.getOfficeVersion();//打开文档后，判断当前office版本
                        return true;
                    }
                    else {
    ...
    this.hiddenSaveLocal = function (httpclient, webOffice, isHidden, isInsertFile, OtherName) {
        ...
        if (httpclient.Status == 200) {
            ...
            if (OtherName == undefined || OtherName == "") {
                ...
                OtherName = this.getUUID() + this.FileType;
            }

            this.DownloadedFileTempPathName = filePath + tempName + OtherName;
            // alert(this.DownloadedFileTempPathName);
    ...
    this.setShowDialog = function (thisType, exts) {
        switch (thisType) {
            ...
            case this.ShowDialog.DialogSaveAs: //另存为
                this.blnIE() ? this.obj.ShowDialog(thisType) : this.obj.ExecuteScript(thisType + "", "ActiveObject.ShowDialog(" + thisType + ")");
                break; //打开窗口
            default:
    ...
    */
    ...
    /* 通过EditType解除iWebOffice2009锁定文档*/
    ...
    this.EditType1 = function () {
        this.obj.EditType = "1, 1";
    };

    this.EditType2 = function () {
        this.obj.EditType = "1,0";
        //alert("11111111");
    };
    ...
    //作用：显示操作状态
    this.StatusMsg = function (mString) {
        this.value = mString;
    };

    /*设置书签植*/
    this.WebShowBookmarks = function () {
        try {
            this.obj.FuncExtModule.WebUrl = this.WebUrl;
            this.obj.FuncExtModule.SetDocument(this.obj.ActiveDocument);
            this.obj.FuncExtModule.WebOpenBookmarks();    //交互OfficeServer的OPTION="LISTBOOKMARKS"

            /*if ((window.ActiveXObject!=undefined) || (window.ActiveXObject!=null) ||"ActiveXObject" in window){
                 this.obj.FuncExtModule.WebOpenBookmarks();
        } else{
                this.obj.ExecuteScript("WebOpenBookmarks", "ActiveObject.FuncExtModule.WebOpenBookmarks()");
        }*/

            StatusMsg(this.obj.FuncExtModule.Status);
        }
        catch (e) {
            alert(e.description);
        }
    ...
    //书签是否存在
    this.WebISHasBookMarks = function(BMarksName){
    ...
    this.WebGetDocSuffix = function () {
        try {
            ...
            if (docType == this.DocType.EXCEL) { //Execl 获取方法
    ...
    //下载文档
    this.DownloadFromRemote = function (DownFileName, SavePath) {
        var httpclient = this.obj.Http;
        httpclient.ShowProgressUI = false;//隐藏进度条
        httpclient.Clear();
        if (httpclient.Open(this.HttpMethod.Get, this.WebUrl, false)) {//指定下载模板的名称
            if (httpclient.Send()) {
                if (httpclient.Status == 200) {
                    httpclient.ResponseSaveToFile(SavePath + DownFileName);
                    httpclient.Clear();
    ...
    */
    ...
    this.VBAProtectDocument = function (docType, password) {
        ...
        if (docType == this.DocType.EXCEL) {//word 保护模式，这里只保护表单1的其他的安自己需求编写
    ...
    //设置书签
    this.VBASetBookmarks = function (BookMarkName, BookMarkValue) {
        try {

            var mBookRange = this.obj.ActiveDocument.Bookmarks.Item(BookMarkName).Range;

            mBookRange.Text = BookMarkValue;
            this.obj.ActiveDocument.Bookmarks.Add(BookMarkName, mBookRange);
        } catch (e) {
            return false;
        }
    };
    ...
    this.VBASetUserName = function (UserName) {
        if(this.FileType != ".ppt" && this.FileType != ".pptx"){
            try {
                this.WebObject.ActiveDocument.Application.Options.UseLocalUserInfo = true;
    ...
    this.WebSavePDF = function () {
        ...
        this.WebSetMsgByName("FILENAME", "公文标题-" + date + ".pdf");//加载FileName
        if(this.SaveAsPdf(this.getFilePath() + "公文标题-" + date + ".pdf")){
            if (this.SAVEFILE(httpclient, this.getFilePath() + "公文标题-" + date + ".pdf")) {
                this.Status = "保存文件成功";
                //alert("保存PDF成功!");
                return true;
            }
    ...
    this.WebDocumentCompare = function (FileName1, FileName2) {
        ...
        var saveFilePath = filePath + this.getUUID() + '.doc';
